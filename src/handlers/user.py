...
from common.behavior_tree import BehaviorTree, BehaviorTreeContext, ConditionNode, ActionNode, SequenceNode
...
class UserCommandHandler:
    ...
    async def _send_message(self, message: Message, text: str) -> bool:
        await message.answer(text)
        return True

    async def _send_sticker(self, message: Message, sticker: str) -> bool:
        await message.answer_sticker(sticker)
        return True
    
    async def random_behavior(self, message : Message):
        
        condition_node = ConditionNode(lambda: message.text is not None)
        ...
        actions = [
        lambda: self._send_message(message, final_first_message), 
        lambda: self._send_message(message, "ну что ты, кит ты мой?"),
        lambda: self._send_sticker(message, random.choice(self.stickers))]
        
        self.behaviorTree.update(SequenceNode([condition_node, ActionNode(random.choice(actions))]))
        await self.behaviorTree.run()

    async def good_night(self, message : Message):
        
        now = datetime.now().hour
        condition_node = ConditionNode(lambda: message is not None)
        if now < 21:
            ...
            self.behaviorTree.update(SequenceNode([
                condition_node,
                ActionNode(lambda: self._send_message(message, random.choice(answers)))
            ]))
            await self.behaviorTree.run()
            return True
        ...
        good_night_message = ActionNode(lambda: self._send_message(message, final_night_message))
        good_night_sticker = ActionNode(lambda: self._send_sticker(message, random.choice(self.stickers)))
        ...
        second_night_message = ActionNode(lambda: self._send_message(message, second_final_night_message))
            
        kiss_messages = ["целую!!", "целуюю!", "целуюююю!!", "целую!!!"]
            
        second_night_message = ActionNode(lambda: self._send_message(message, second_final_night_message))
        third_night_message = ActionNode(lambda: self._send_message(message, random.choice(kiss_messages)))

        last_message = random.choice([
                ActionNode(lambda: self._send_message(message, "не сиди долго!")),
                good_night_sticker
        ])
    ...
    async def good_morning(self, message: Message):
        ...
        if now > 5 and now < 12:
            ...
            introductory = ActionNode(random.choice([lambda: self._send_message(message, random.choice(introductory_questions)),
                                            lambda: self._send_message(message, random.choice(moods))]))
            
            final_morning_message = f"{random.choice(morning_greetings)} {random.choice(self.nicknames)} {random.choice(self.emojies)}"
            
            send_morning = ActionNode(lambda : self._send_message(message, final_morning_message))
            
            
            self.behaviorTree.update(SequenceNode([condition_node, 
                                                                send_morning, 
                                                                introductory]), BehaviorTreeContext())
            await self.behaviorTree.run()
        else:
